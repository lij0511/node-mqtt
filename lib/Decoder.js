  ...
  var decoders = {
    ...
    'CONNECT' : function() {
      ...
      if(_count >= remainingLength) {
        throw("Protocol error - data length error");
      }
      ...
      /* client id length */
      var clientIdLength = _getLength();
      if(!clientIdLength || clientIdLength > 23) {
        message.error = 'identifier rejected';
        message.ack = 2;
        return message;
      }
      if(clientIdLength + _count > data.length) {
        throw('Protocol error - client ID length');
      }
      message.clientId = data.slice(_count, _count + clientIdLength).toString('utf8');
      _count += clientIdLength;
      ...
      if(message.willFlag) {
        ...
        message.willTopic = data.slice(_count, _count + topicLen).toString('utf8');
        _count += topicLen;
        var messageLen = _getLength();
        message.willMessage = data.slice(_count, _count + messageLen);
        _count += messageLen;
      }
      ...
      if(message.hasUsername) {
        ...
        message.username = data.slice(_count, _count + usernameLen).toString('utf8');
      }
      if(message.hasPassword) {
        var passwordLen = _getLength();
        message.password = data.slice(_count, _count + passwordLen).toString('utf8');
      }
      /* decode payload end */
