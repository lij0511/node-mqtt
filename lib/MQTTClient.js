...
MQTTClient.prototype.listenEvent = function() {
  ...
  this._socket.on('data', function(data) {
    self.buffer = utils.concat(self.buffer, data);
    self.parse();
  });
  /* error event */
 ...
 * @param data
 */
MQTTClient.prototype.parse = function() {
  try {
    var message = decoder.decode(this.buffer);
    if(message.status == 1){
    ...
    this.buffer = this.buffer.slice(trunkLength);
